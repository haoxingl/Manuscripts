...
Research manuscripts by Haoxing Lin (NUS School of Computing),
advised by Prof. Prashant Nalini Vasudevan.
...
by prior analyses, and includes an efficient algorithm for computing provably
correct bounds for concrete parameters. Also analyzes the k-Tree algorithm
over ℤ_m with extensive empirical evaluation.  
...
throughout all levels of computation.  
...
denominator lower-bound via a second-moment argument over projective lines.
Identifies a precise obstruction to this Fourier-analytic route: the final
mass condition forces m − n = Ω(n log n / log log n), whereas the target
Wagner/SIS regime requires only m − n = ω(n / log log n).  
